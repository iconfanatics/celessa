...
- **Rich Text Contrast Fix**: Added a CSS override in `assets/theme.scss.liquid` (`.rte span { color: $colorTextBody !important; }`) to ensure that any text pasted into the Shopify rich text editor with light inline font colors correctly inherits the theme's high-contrast body text color, fixing Lighthouse contrast ratio errors on product descriptions.
